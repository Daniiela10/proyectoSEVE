...
router.get('/todos', authMidd, async (req, res) => {
  try {
    if (!(await usuarioEsAdmin(req.usuario.id))) return res.status(403).json({ error: 'Sin permisos' });
    const pedidos = await Pedido.find().populate('usuario', 'nombres apellidos email').sort({ createdAt: -1 });
    res.json(pedidos);
  } catch (err) {
...
router.patch('/:id/envio', authMidd, async (req, res) => {
  try {
    ...
    if (!pedido) return res.status(404).json({ error: 'Pedido no encontrado' });
    if (pedido.estado !== 'despachado') {
      return res.status(400).json({ error: 'Solo puedes registrar envio para pedidos despachados' });
    }
