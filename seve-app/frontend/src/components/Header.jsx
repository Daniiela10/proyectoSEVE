...
const PROMO_ITEMS = [
  ...
  "Hasta 30% de descuento en juegos de ollas",
  "Paga en cuotas sin interés",
  "Atención al cliente Lun–Sab 8am–6pm",
];

export default function Header({ onAbrirCarrito }) {
  const {
    usuario, cerrarSesion, setVista, cantidadCarrito,
    vista, setBusqueda, productos, cartPulseKey,
  } = useApp();
          ...
          <nav className="nav-principal">
            <ul>
              {[
                ...
                { vista: "mayor", label: "Ventas X Mayor" },
              ].map(({ vista: v, label }) => (
              ...
              <li className="nav-cliente">
                <a
                  href="#"
                  className={`nav-link nav-carrito${carritoAnimado ? " is-bumping" : ""}`}
                  onClick={(e) => {
                    e.preventDefault();
              ...
              {menuPerfilAbierto && (
                <div className="perfil-menu" style={{
                  ...
                  <div style={{ padding: "8px" }}>
                    <button type="button" onClick={() => { setVista("perfil"); setMenuPerfilAbierto(false); }} style={profileMenuBtnStyle}>
                      Editar mi perfil
                    </button>
                    {!esStaff && (
                      <button type="button" onClick={() => { setVista("historial"); setMenuPerfilAbierto(false); }} style={profileMenuBtnStyle}>
                        Mis pedidos
                      </button>
                    )}
                    <button type="button" onClick={() => { cerrarSesion(); setMenuPerfilAbierto(false); }} style={{ ...profileMenuBtnStyle, color: "#f40808" }}>
                      Cerrar sesión
                    </button>
                  </div>
                </div>
            ...
            <ul className="menu-mobile-ul">
              {[
                ...
                <li key={v}>
                  <a
                    href="#"
                    className={`menu-mobile-link${vista === v ? " active" : ""}`}
            ...
            <div className="menu-mobile-actions">
              {!usuario ? (
                ...
                <div className="menu-mobile-user">
                  ...
                  <button type="button" className="menu-mobile-link" onClick={() => { cerrarSesion(); setMenuMobileAbierto(false); }} style={{ color: "#f40808" }}>
                    Cerrar sesión
                  </button>
                </div>
              )}
